...
my $NOTE = "Generated by mest -0.9.5";
...
my $columnsText = "*{" . ($eqc + 1) . "}{\\columnFormat}";
...
# precompute a shuffled order in which alternatives are chosen
my @shuffleAlternatives = Shuffle (0..$vc-1);

# shuffle each set of alternatives
for(my $i = 0; $i < $eqc; ++$i) {
    # shuffle only if a set of alternatives
    if (reftype @{$questions[$i]}[0]) {
	@{$questions[$i]} = Shuffle @{$questions[$i]};
    }
}
...
for(my $variant = 1; $variant <= $vc; ++$variant){
	...
	# post process - replace here ...
        ...
        # dirty hack below: sneak in a Trojan horse flag into the variant number
